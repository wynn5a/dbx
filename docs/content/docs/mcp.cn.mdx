...
MCP 查询支持 PostgreSQL、MySQL 及兼容数据库。打开表等 UI 联动能力支持当前 DBX 桌面端可以打开的所有数据库类型。
...
MCP 工具始终面向本机的 DBX 桌面应用工作。查询类工具直接读取桌面端本地存储，`dbx_open_table`、`dbx_execute_and_show` 等 UI 工具则通过桌面 bridge（运行中的 DBX 应用提供的本地 HTTP 接口）调用桌面应用。
...
<Accordions>
  ...
  <Accordion title="dbx_open_table 报 'DBX is not running'">
    需要先启动 DBX 桌面应用。UI 联动通过 DBX 的本地 HTTP bridge 完成：应用启动时自动选择空闲端口，并写入数据目录下的 `mcp-bridge-port` 文件。
  </Accordion>
...
- [DBX](https://github.com/wynn5a/dbx) 已安装并配置了至少一个数据库连接
- Node.js 18+
