---
title: 什么是 DBX？
description: DBX 是一个开源桌面数据库工作台，覆盖连接管理、SQL 编辑、数据浏览、结构工具和 AI 助手。
---

DBX 是一个开源数据库工作台。它把日常数据库工作里的高频动作放在同一个界面里：连接数据库、写 SQL、查看和编辑数据、浏览结构、对比 Schema、导入导出数据，以及在需要时让 AI 辅助分析。

DBX 以桌面应用的形式发布，支持 macOS、Windows 和 Linux。安装到本机后，即可在原生窗口中管理你的所有数据库。
...
- **本地优先**：所有能力都在原生桌面应用中运行，连接配置和数据都留在你自己的机器上。
