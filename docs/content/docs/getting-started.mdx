...
<Tabs groupId="platform" items={['macOS', 'Windows', 'Linux', 'Docker']}>
  ...
  <Tab value="Docker">
    Docker is for self-hosting DBX on a server and opening it from a browser. Build and run it from source with the compose file shipped in the repository:
    ...
    Then open `http://localhost:4224`. The web password is set via `DBX_PASSWORD` in `deploy/docker-compose.yml` — change it before exposing the server.
  </Tab>
</Tabs>
