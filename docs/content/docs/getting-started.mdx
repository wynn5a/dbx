...
description: Install the DBX desktop app, create your first connection, and understand the desktop and source workflows.
---

This guide gets you through three things:

1. Install the DBX desktop app
...
<Tabs groupId="platform" items={['macOS', 'Windows', 'Linux']}>
  <Tab value="macOS">
    Download the `.dmg` installer from [GitHub Releases](https://github.com/wynn5a/dbx/releases) and drag DBX into Applications.
  </Tab>
  <Tab value="Windows">
    Download the `.msi` installer from [GitHub Releases](https://github.com/wynn5a/dbx/releases) and run it.
  </Tab>
  <Tab value="Linux">
    Download a package from [GitHub Releases](https://github.com/wynn5a/dbx/releases):
...
| Aspect | Description |
|---|---|
| Best For | Daily local work, local files, native windows, DBX UI opened from MCP |
| Backend Path | Tauri commands calling the Rust core |
| Storage | Local app data directory |

<Callout type="info">
The DBX MCP server talks to the desktop app, so keep DBX running while you use MCP tools such as opening DBX windows from your AI assistant.
</Callout>
...
| File database cannot open | File path, file permissions, or unsupported file extension |
| Private database unreachable | Configure SSH tunnel, proxy, VPN, or run DBX on a machine that can reach the database |
...
```bash
git clone https://github.com/wynn5a/dbx.git
cd dbx
pnpm install
pnpm dev:tauri
```
