...
MCP query support is implemented for PostgreSQL, MySQL, and compatible databases. UI integration such as opening a table supports all database types that the running DBX desktop app can open.
...
MCP tools always run against the DBX desktop app on your machine. Query tools read local desktop storage directly, while UI tools such as `dbx_open_table` and `dbx_execute_and_show` call the desktop bridge — the local HTTP interface exposed by the running DBX app.
...
<Accordions>
  ...
  <Accordion title="dbx_open_table says 'DBX is not running'">
    Start the DBX desktop app first. UI integration goes through DBX's local HTTP bridge — the app picks a free port at startup and writes it to the `mcp-bridge-port` file in the DBX data directory.
  </Accordion>
