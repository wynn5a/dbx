...
<template>
  <DsDialog
    v-model:open="open"
    :title="updateInfo?.update_available ? t('updates.availableTitle') : t('updates.title')"
    :icon="Download"
    content-class="sm:max-w-[520px]"
    @interact-outside="
      (e: Event) => {
  ...
  >
    <div class="space-y-3 text-sm">
      ...
      <p v-else class="text-[var(--ds-text-3)]">
      ...
      <div
        ...
        class="max-h-48 overflow-auto rounded-md border border-[var(--ds-border-soft)] bg-[var(--ds-bg-canvas)] p-3 text-xs [&_h1]:text-sm [&_h1]:font-semibold [&_h1]:mb-1 [&_h2]:text-sm [&_h2]:font-semibold [&_h2]:mb-1 [&_h3]:text-xs [&_h3]:font-semibold [&_h3]:mb-1 [&_ul]:list-disc [&_ul]:pl-4 [&_ul]:my-1 [&_ol]:list-decimal [&_ol]:pl-4 [&_ol]:my-1 [&_li]:my-0.5 [&_p]:my-1 [&_code]:bg-[var(--ds-bg-canvas)] [&_code]:px-1 [&_code]:py-0.5 [&_code]:rounded [&_code]:text-[11px] [&_a]:text-primary [&_a]:underline"
      ...
      <template v-if="updateInfo?.update_available">
        ...
        <template v-if="canDownloadAndInstallUpdate(updateInfo, isDesktop)">
          <div v-if="updateReady" class="flex flex-col items-end gap-1">
            ...
            <span class="text-xs text-[var(--ds-text-3)]">{{ t("updates.reopenHint") }}</span>
          </div>
