...
- [x] **MEDIUM — the palette bypasses the toolbar's enable/disable rules**
  ...
  - Fixed in `49fe5fad`: `enabled` predicates + `runPaletteCommand`; disabled rows render `aria-disabled` and Enter/click do nothing (palette stays open). Tests: `commandPalette.test.ts` (no-connection stub context).
- [x] **LOW — the palette is a second action registry, separate from the shortcut registry**
  ...
  - Fixed in `49fe5fad`: `shortcutId` + `commandShortcutHint` (live binding via `formatShortcut`); Format SQL, Refresh Data, Toggle Sidebar, Next/Prev Tab added. New Connection / Settings / Toggle Sidebar / Refresh are shared App handlers used by toolbar, keydown and palette (New Query already was `newQuery()` everywhere).
- [x] **LOW — Mod+K opens the palette on top of other modal dialogs**
  ...
  - Fixed in `49fe5fad`: `isModalDialogOpen` (open dialog/sheet content or alert dialog; not `[role="dialog"]`, which popovers also carry) + `resolveCommandPaletteToggle` — Mod+K still closes an open palette but is ignored over other modals. The Settings shortcut got the same guard.
- [x] **LOW (cleanup) — `watch(open)` in `CommandPalette.vue` never fires**
  ...
  - Fixed in `49fe5fad`: watch removed (component is remounted per open).
