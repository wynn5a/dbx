...
function connectionStoreHint(message: string): string | undefined {
  if (/NODE_MODULE_VERSION|compiled against a different Node\.js version/i.test(message)) {
    return "Rebuild DBX native dependencies with your active Node.js: pnpm rebuild better-sqlite3 --pending, or reinstall the package with the same Node.js version you use to run the MCP server.";
  }
  return undefined;
}
